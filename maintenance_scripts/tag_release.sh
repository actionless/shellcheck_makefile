...
set -euo pipefail
IFS=$'\n\t'
pkgname=shellcheck_makefile
#aur_repo_dir=~/build/$pkgname
aur_dev_repo_dir=~/"build/${pkgname}-git"
...
if [[ -n "${aur_repo_dir:-}" ]] ; then
	...
	sed \
		-e "s|pkgname=${pkgname}-git|pkgname=${pkgname}|" \
		-e 's|"$pkgname::git+https://github.com/actionless/'"$pkgname"'.git#branch=master"|"$pkgname-$pkgver.tar.gz"::https://github.com/actionless/'"$pkgname"'/archive/"$pkgver".tar.gz|' \
		-e "s|conflicts=('${pkgname}')|conflicts=('${pkgname}-git')|" \
